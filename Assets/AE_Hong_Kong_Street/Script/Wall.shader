...
Shader "AE/Wall"
{
	Properties
	{
		...
		[ASEBegin]_Color_01("Color_01", Color) = (0.2180491,0.9433962,0.2187248,0)
		...
		[ASEEnd]_Wall_Mask("Wall_Mask", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Wall_Mask_ST;
			float4 _Color_02;
			float4 _Wall_01_A_ST;
			float4 _Color_01;
			float4 _Wall_01_N_ST;
			float4 _Wall_01_MS_ST;
			float _Normal_Power;
			float _Smoothness;
			...
			#endif
			CBUFFER_END
			sampler2D _Wall_Mask;
			sampler2D _Wall_01_A;
			sampler2D _Wall_01_N;
			sampler2D _Wall_01_MS;
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_Wall_Mask = IN.ase_texcoord7.xy * _Wall_Mask_ST.xy + _Wall_Mask_ST.zw;
				float4 tex2DNode2 = tex2D( _Wall_Mask, uv_Wall_Mask );
				float2 uv_Wall_01_A = IN.ase_texcoord7.xy * _Wall_01_A_ST.xy + _Wall_01_A_ST.zw;
				float4 tex2DNode1 = tex2D( _Wall_01_A, uv_Wall_01_A );
				
				float2 uv_Wall_01_N = IN.ase_texcoord7.xy * _Wall_01_N_ST.xy + _Wall_01_N_ST.zw;
				float3 unpack12 = UnpackNormalScale( tex2D( _Wall_01_N, uv_Wall_01_N ), _Normal_Power );
				unpack12.z = lerp( 1, unpack12.z, saturate(_Normal_Power) );
				
				float2 uv_Wall_01_MS = IN.ase_texcoord7.xy * _Wall_01_MS_ST.xy + _Wall_01_MS_ST.zw;
				
				float3 Albedo = ( ( tex2DNode2 * _Color_02 * tex2DNode1 ) + ( tex2DNode1 * ( ( 1.0 - tex2DNode2 ) * _Color_01 ) ) ).rgb;
				float3 Normal = unpack12;
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = 0;
				float Smoothness = ( _Smoothness * tex2D( _Wall_01_MS, uv_Wall_01_MS ).a );
				float Occlusion = 1;
				float Alpha = 1;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv_Wall_Mask = IN.ase_texcoord2.xy * _Wall_Mask_ST.xy + _Wall_Mask_ST.zw;
				float4 tex2DNode2 = tex2D( _Wall_Mask, uv_Wall_Mask );
				float2 uv_Wall_01_A = IN.ase_texcoord2.xy * _Wall_01_A_ST.xy + _Wall_01_A_ST.zw;
				float4 tex2DNode1 = tex2D( _Wall_01_A, uv_Wall_01_A );
...
/*ASEBEGIN
Version=18935
-1913;73;1920;941;1685.42;522.5145;1.386506;True;False
...
ASEEND*/
//CHKSM=F877D92A53337BBB0A45BD072D6A3E79DC5D5A8C